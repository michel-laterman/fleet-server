...
func TestParsedPolicyOTELSecretsReplacement(t *testing.T) {
	...
	otlpMap := pp.Policy.Data.Receivers["otlp"].(map[string]interface{})
	require.Equal(t, "receiver-auth-id_value", otlpMap["auth"])

	// Validate that path-based secret references were replaced in exporters
	otlphttpMap := pp.Policy.Data.Exporters["otlphttp/default"].(map[string]interface{})
	require.Equal(t, "exporter-auth-id_value", otlphttpMap["headers"].(map[string]interface{})["authorization"])

	// Validate that inline secret references were replaced in processors
	batchMap := pp.Policy.Data.Processors["batch"].(map[string]interface{})
	...
	basicauthMap := pp.Policy.Data.Extensions["basicauth"].(map[string]interface{})
	...
	spanmetricsMap := pp.Policy.Data.Connectors["spanmetrics"].(map[string]interface{})
